export default async function handler(req, res) {
    ...
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;">
      ...
      <div style="padding:32px;background:#f5f2eb;">
        ...
        <div style="margin-top:20px;border-top:1px solid #ddd;padding-top:16px;">
          <p style="font-size:11px;color:#999;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:8px;">Electronic Signature &amp; Liability Release</p>
          ${signatureData.startsWith('Typed:')
            ? '<p style="font-family:Georgia,serif;font-style:italic;font-size:22px;color:#0a1628;margin:0;">' + signatureData.replace('Typed: ','') + '</p>'
            : '<img src="' + signatureData + '" style="max-width:100%;border:1px solid #eee;border-radius:2px;" alt="Customer signature">'
          }
          <p style="font-size:11px;color:#999;margin-top:6px;">Signed at checkout — customer agreed to research-use-only terms and full liability release.</p>
        </div>` : ''}
      </div>
    </div>`;
  ...
  const buyerEmailHTML = `
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;">
      ...
      <div style="padding:32px;background:#f5f2eb;">
        <p style="font-size:15px;color:#333;margin-bottom:24px;">Hi ${first}, thank you for your research order! We have received it and will be in touch within 24 hours to confirm and arrange payment.</p>
        <table style="width:100%;border:1px solid #ddd;background:#fff;margin-bottom:24px;">
          <thead>
            <tr style="background:#0a1628;">
              <th style="padding:10px 12px;color:#fff;text-align:left;font-size:12px;letter-spacing:0.1em;">ITEM</th>
